  ...
  useEffect(() => {
    const setDailyMemos = () => {
      const dailyMemos = memoService
        ...
        .memos.filter(
          (a) =>
            utils.getTimeStampByDate(a.created_at) >= currentDateStamp &&
            utils.getTimeStampByDate(a.created_at) < currentDateStamp + DAILY_TIMESTAMP
        )
        .sort((a, b) => utils.getTimeStampByDate(a.created_at) - utils.getTimeStampByDate(b.created_at));
      setMemos(dailyMemos);
      ...
      <div className="dialog-content-container" ref={memosElRef}>
        ...
        {loadingState.isLoading ? (
          ...
          <div className="dailymemos-wrapper">
            {memos.map((memo) => (
              <DailyMemo key={`${memo.id}-${memo.updated_at}`} memo={memo} />
            ))}
          </div>
        )}
      </div>
