...
const MemoCardDialog: React.FC<Props> = (props: Props) => {
  const [memo, setMemo] = useState<FormattedMemo>({
    ...props.memo,
    createdAtStr: utils.getDateTimeString(props.memo.created_at),
  });
  ...
  useEffect(() => {
    const fetchLinkedMemos = async () => {
      try {
        ...
        for (const matchRes of matchedArr) {
          if (matchRes && matchRes.length === 3) {
            ...
            if (memoTemp) {
              linkMemos.push({
                ...memoTemp,
                createdAtStr: utils.getDateTimeString(memoTemp.created_at),
                dateStr: utils.getDateString(memoTemp.created_at),
              });
            }
          }
        }
        ...
        setLinkedMemos(
          linkedMemos
            .sort((a, b) => utils.getTimeStampByDate(b.created_at) - utils.getTimeStampByDate(a.created_at))
            .map((m) => ({
              ...m,
              createdAtStr: utils.getDateTimeString(m.created_at),
              dateStr: utils.getDateString(m.created_at),
            }))
        );
      } catch (error) {
        // do nth
