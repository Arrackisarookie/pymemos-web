  ...
  switch (action.type) {
    case "SET_MEMOS": {
      const memos = utils.dedupeObjectWithId(
        action.payload.memos.sort((a, b) => utils.getTimeStampByDate(b.created_at) - utils.getTimeStampByDate(a.created_at))
      );
    ...
    case "INSERT_MEMO": {
      const memos = utils.dedupeObjectWithId(
        [action.payload.memo, ...state.memos].sort((a, b) => utils.getTimeStampByDate(b.created_at) - utils.getTimeStampByDate(a.created_at))
      );
